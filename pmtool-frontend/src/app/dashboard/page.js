  ...
  useEffect(() => {
    ...
    const fetchStatsAndActivities = async () => {
      try {
        ...
        const membersArray = Array.isArray(membersRes.data?.data)
          ? membersRes.data.data
          : Array.isArray(membersRes.data)
          ? membersRes.data
          : [];
        const projectsArray = Array.isArray(projectsRes.data?.data)
          ? projectsRes.data.data
          : Array.isArray(projectsRes.data)
          ? projectsRes.data
          : [];
        const tasksArray = Array.isArray(tasksRes.data?.data)
          ? tasksRes.data.data
        ...
        const memberActs = membersArray.slice(-5).map((m) => ({
          id: `member-${m.id}`,
          text: `${m.username} joined the team`,
        }));

        const projectActs = projectsArray.slice(-5).map((p) => ({
          id: `project-${p.id}`,
          text: `Project "${p.name}" was created`,
        }));

        const taskActs = tasksArray.slice(-5).map((t) => ({
          id: `task-${t.id}`,
        ...
        setStats({
          ...
          changeLogs: membersArray.length + projectsArray.length + tasksArray.length,
